/**
 * Modal.js
 * Componente para el modal de detalles de películas
 */
...
/**
 * Componente Modal para mostrar detalles de películas
 */
export class Modal {
    /**
    ...
    constructor(modalElement) {
        if (!modalElement) {
            throw new Error('Modal requiere un elemento DOM válido');
        }
        this.modal = modalElement;
        this.modalBody = document.getElementById('modal-body');

        // Referencias a los elementos de botones actuales
        this.currentFavoriteBtn = null;
        this.currentWatchedBtn = null;
        this.currentFavoriteHandler = null;
        this.currentWatchedHandler = null;
        this.currentSimilarHandlers = [];

        modalLogger.info('🎭 Modal component inicializado');
    }

    /**
     * Abre el modal con los detalles de la película
     * @param {Object} details - Detalles completos de la película
     */
    open(details) {
    ...
    setupEventListeners(details) {
        ...
        this.currentFavoriteBtn = this.modalBody.querySelector('[data-action="favorite"]');
        this.currentWatchedBtn = this.modalBody.querySelector('[data-action="watched"]');

        // Crear handlers
        this.currentFavoriteHandler = () => {
            ...
            this.open(details); // Re-abrir para actualizar estado
            this.modal.dispatchEvent(new CustomEvent('movie-state-changed'));
        };

        this.currentWatchedHandler = () => {
        ...
        if (this.currentFavoriteBtn) {
            this.currentFavoriteBtn.addEventListener('click', this.currentFavoriteHandler);
            modalLogger.debug('✓ Listener de favoritos agregado');
        }

        if (this.currentWatchedBtn) {
            this.currentWatchedBtn.addEventListener('click', this.currentWatchedHandler);
            modalLogger.debug('✓ Listener de vistas agregado');
        }

        // Películas similares
        const similarElements = this.modalBody.querySelectorAll('.similar-movie');
        ...
        similarElements.forEach(el => {
            ...
            this.currentSimilarHandlers.push({ element: el, handler });
        });
    ...
    cleanupEventListeners() {
        ...
        if (this.currentFavoriteBtn && this.currentFavoriteHandler) {
            this.currentFavoriteBtn.removeEventListener('click', this.currentFavoriteHandler);
            modalLogger.debug('🧹 Listener de favoritos removido');
        }

        if (this.currentWatchedBtn && this.currentWatchedHandler) {
            this.currentWatchedBtn.removeEventListener('click', this.currentWatchedHandler);
            modalLogger.debug('🧹 Listener de vistas removido');
        }

        // Remover listeners de películas similares
        if (this.currentSimilarHandlers.length > 0) {
            modalLogger.debug(`🧹 Limpiando ${this.currentSimilarHandlers.length} event listeners de similares`);
            this.currentSimilarHandlers.forEach(({ element, handler }) => {
